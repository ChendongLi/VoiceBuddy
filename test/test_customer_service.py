...
class TestCustomerService:
    ...
    async def test_get_or_create_new(self, db_session, customer_svc):
        customer, is_new = await customer_svc.get_or_create(db_session, "test-tenant", "+15559999999")
    ...
    async def test_update_profile(self, db_session, customer_svc):
        ...
        updated = await customer_svc.update_profile(db_session, customer.id, name="Sarah", email="sarah@example.com")
    ...
    async def test_update_profile_partial(self, db_session, customer_svc):
        ...
        updated = await customer_svc.update_profile(db_session, customer.id, contact_pref="sms")
        assert updated.name == "Sarah"
    ...
    def test_build_context_returning_with_appointment(self, customer_svc):
        ...
        apt = Appointment(
            ...
            starts_at=datetime(2025, 1, 15, 10, 0, tzinfo=UTC),
            service_name="Cleaning",
            provider_name="Dr. Smith",
            duration_min=60,
        )
...
class TestCallService:
    ...
    async def test_start_call(self, db_session, call_svc, customer_svc):
        customer, _ = await customer_svc.get_or_create(db_session, "test-tenant", "+15559999999")
        call = await call_svc.start_call(db_session, "test-tenant", customer.id, "CA123", "+15559999999")
    ...
    async def test_end_call(self, db_session, call_svc, customer_svc):
        customer, _ = await customer_svc.get_or_create(db_session, "test-tenant", "+15559999999")
        call = await call_svc.start_call(db_session, "test-tenant", customer.id, "CA456", "+15559999999")
